...
import { POST } from '@/app/api/documents/ninety-day-plan/route';
...
const URL = 'http://localhost/api/documents/ninety-day-plan';

beforeEach(() => {
  vi.clearAllMocks();
});

describe('POST /api/documents/ninety-day-plan', () => {
  ...
  it('returns contract-shape DocumentResponse on success', async () => {
    ...
    mPrisma.document.create.mockResolvedValueOnce(fakeDocumentRow({ kind: 'ninety_day' }));
    ...
    expect(body.kind).toBe('ninety_day');
  });
});
